...
- **What does `0` mean in the command related to permissions, e.g. `chmod 0777`?**  
It has few meanings, depending on the context, the command it is used with, etc.  
In a PHP program, in a C program or similar, `0777` is octal (three sets of three 1 bits, e.g. `7` in octal means `111` in binary, e.g. `77` in octal means `111111` in binary, e.g. `777` in octal means `111111111` in binary), while "a simple" `777` (not `0777`!) means decimal, and it's quite a different bit pattern. So, in many of the cases the first leading zero means the number given after that is octal.  
However, the leading `0`in `chmod` case is a bit of a different story. `chmod` will interpret any numeric argument as octal, hence no leading zero is necessary. But this is not all to the case of `chmod`. From the `man chmod`: *"A numeric mode is from one to  four  octal  digits  (0-7),  derived  by adding up the bits with values 4, 2, and 1.  Omitted digits are assumed to be leading zeros.  The first digit selects the set user ID  (4)  and set group ID (2) and restricted deletion or sticky (1) attributes.  The second digit selects permissions for the user who owns the  file:  read (4),  write  (2),  and  execute  (1); the third selects permissions for other users in the file's group, with the same values; and  the  fourth for other users not in the file's group, with the same values."*  
...
https://superuser.com/questions/592309/whats-the-difference-between-chmod-777-and-chmod-7777  
https://ss64.com/bash/chmod.html  
https://ss64.com/bash/sticky.html  
...
Sends file `mytestfile.txt` from the current working dirrectory to `10.140.16.201` server (as seen from bastion server) via the middle server `bastion.example.com`.
TBD  
...
Copy file `/srv/www/example.com/logs/dev.log` of `10.140.16.201` server (as seen from bastion server) to local `/home/paulius/dev/scp-tests/backup_logs/dev.log` via the middle server `bastion.example.com`  
TBD  
...
- **Explain `sort myfile.json | awk '{$1=$1;print}' | uniq --count | sort -n`**  
...
Setting reserved block percentage to 1%. By default, Linux reserves 5% of the drive for the core operations (e.g. in case the drive is full, this space is in use for commands to operate on).  
